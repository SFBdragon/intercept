...
impl Collider {
   pub fn new_reacter(body: Body, reacter: Box<dyn Reacter>) -> Collider {
      ...
      Collider {
         ...
         is_static: false,
         ctu: ColliderTypeUnion { reacter: ManuallyDrop::new(reacter) },
         broad_y: (broad.min.y, broad.max.y),
         remainder: 1.0,
      }
   }
   pub fn new_reacter_trigger(body: Body, trigger: Box<dyn Fn(usize, usize, f64, BodySweepData, f64)>, reacter: Box<dyn Reacter>) -> Collider {
      let broad = body.get_broad();
      Collider {
         body,
         trigger,
   ...
   /// Knock defines whether dynamic colliders should have this static collider's velocity added to them on collision. 
   /// This is usually recommended to be enabled for collisions to appear more sensible.
   pub fn new_static_trigger(body: Body, trigger: Box<dyn Fn(usize, usize, f64, BodySweepData, f64)>, knock: bool) -> Collider {
      let broad = body.get_broad();
      Collider {
         body,
         trigger,
...
impl Plane {
   ...
   pub fn update(&mut self) {
      ...
      if self.require_resort { self.sort_be_vals(); }
         ...
         match val.0 {
            ...
            true => {
               ...
               let (mut was_candidate, (mut id2, mut val2_be_b, mut data))
                  = if let Some(c) = candidates.remove(&id1) { (true, c) } else {
               ...
               for (v2_id, v2_be_b) in active.iter() {
                  ...
                  if c1.broad_y.0 <= c2.broad_y.1 && c1.broad_y.1 >= c2.broad_y.0 && !(c1.is_static && c2.is_static) { // y broad box & static-static check
               ...
               if id2 != usize::MAX {
                  ...
                  let (c2_is_static, c2_vel_add) = 
                     if c2.is_static { (true, if unsafe { c2.ctu.stat } { c2.body.vel } else { cgmath::vec2(0.0, 0.0) }) } 
                  ...
                  if !c2_is_static {
                     ...
                     let vel = unsafe { (*c1.ctu.reacter).react(c1.body.vel, id1, id2, data, epsilon) };
         ...
         for (be_index, val) in self.be_list.iter().enumerate() {
            ...
            match val.0 {
               ...
               true => {
                  ...
                  if !prev_sort && !was_candidate { continue; }
                  
                  let table_raw: *mut HashMap<usize, Collider, BuildHasherDefault<FnvHasher>> = &mut self.table;
                  // Retrieve the collider.
                  let c1 = unsafe { (*table_raw).get_mut(&id1).expect("Table does not contain Id of val in be_list") };
                  let mut rem_diff = f64::NAN;

                  // Find the closest collision, if any.
                  for (v2_id, v2_be_b) in active.iter() {
                     if prev_sort || to_sort.contains_key(v2_id) { // don't check colliders if niether were sort candidates
                        let c2 = unsafe { (*table_raw).get_mut(&id2).expect("Table does not contain Id of val in active") };
                        if c1.broad_y.0 <= c2.broad_y.1 && c1.broad_y.1 >= c2.broad_y.0 && !(c1.is_static && c2.is_static) { // y broad box check
                           ...
                           if c1c2_rem_diff < 0.0 {
                              ...
                              if let Some(bsd) = crate::narrow::swept::body_sweep(&c1.body, &c2.body, t * c1.remainder) {
                                 if bsd.travel < data.travel {
                                    was_candidate = false;
                                    data = bsd;
                                    id2 = *v2_id;
                              ...
                              if let Some(bsd) = crate::narrow::swept::body_sweep(&c1.body, &c2.body, t * c2.remainder) {
