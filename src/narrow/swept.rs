...
impl Body {
    pub fn new(shapes: Vec<Shape>, pos: Vector2<f64>, vel: Vector2<f64>, is_bullet: bool) -> Body {
        ...
        Body {
            pos,
            vel,
            aabb: Aabb::new(ix, iy, ax, ay).translate(pos),
            shapes,
            bullet: is_bullet,
        }
    }
...
#[inline]
fn aabb_aabb_passby(
    vel_a1a2: Vector2<f64>,
    aabb1: &Aabb,
    aabb2: &Aabb,
    diff_p1p2: Vector2<f64>,
) -> bool {
    // ~4.4ns
    let (l1r2, r1l2) = if vel_a1a2.x >= 0.0 {
        if vel_a1a2.y >= 0.0 {
            (
                aabb2.maxx_miny() - aabb1.minx_maxy(),
                aabb2.minx_maxy() - aabb1.maxx_miny(),
            )
        } else {
    ...
    if seg_origin.x < aabb.min.x - circle.rad && seg_end.x > aabb.min.x - circle.rad {
        ...
        if differentiator >= -circle.rad && differentiator < aabb_height + circle.rad {
            ...
            return if differentiator < 0.0 {
                ...
                if let Some(travel) = (Circle {
                    rad: circle.rad,
                    pos: aabb.min,
                })
                .line_query(seg_origin, seg_end)
                {
                    Some((
                        travel,
                        aabb.min
                            - (seg_origin - rv.mul_element_wise(travel))
                                .div_element_wise(circle.rad),
                    ))
                } else {
                    None
                }
        ...
        if differentiator >= -circle.rad && differentiator < aabb_height + circle.rad {
            if differentiator < 0.0 {
                return if let Some(travel) = (Circle {
                    rad: circle.rad,
                    pos: aabb.max,
                })
                .line_query(seg_origin, seg_end)
...
fn circle_poly_sweep(
    ...
    if (b1.bullet || b2.bullet) && !circle_aabb_passby(unit_rv, &poly.aabb, o, circle.rad) {
        return None;
    }
                ...
                if t < 0.0 {
                    ...
                    if x > 0.0 {
                        ...
                        return if d < rv_mag {
                            ...
                            Some((
                                d,
                                (unit_rv.mul_element_wise(d) - vo).div_element_wise(circle.rad),
                            ))
                        } else {
                            None
                        };
                    ...
                    if x > 0.0 {
                        ...
                        return if d < rv_mag {
                            Some((
                                d,
                                (unit_rv.mul_element_wise(d) - v1o).div_element_wise(circle.rad),
                            ))
                        } else {
                            None
                        };
...
fn shape_sweep(b1: &Body, s1: usize, b2: &Body, s2: usize, t: f64) -> Option<(f64, Vector2<f64>)> {
    ...
    fn aabb_circle_sweep_inter(
        ...
        unsafe { inv_norm!(circle_aabb_sweep(b2, &su2.circle, b1, &su1.aabb, t)) }
    }
    fn aabb_poly_sweep_inter(
        b1: &Body,
        su1: &ShapeUnion,
        b2: &Body,
        su2: &ShapeUnion,
    ...
    fn poly_circle_sweep_inter(
        ...
        unsafe { inv_norm!(circle_poly_sweep(b2, &su2.circle, b1, &su1.poly, t)) }
    }
    fn poly_aabb_sweep_inter(
        b1: &Body,
        su1: &ShapeUnion,
        b2: &Body,
        su2: &ShapeUnion,
        ...
        unsafe { inv_norm!(aabb_poly_sweep(b2, &su2.aabb, b1, &su1.poly, t)) }
    }

    type InterSweep =
        fn(&Body, &ShapeUnion, &Body, &ShapeUnion, f64) -> Option<(f64, Vector2<f64>)>;
    const SHAPE_JUMP_TABLE: [[InterSweep; 3]; 3] = [
        [
            circle_circle_sweep_inter,
            circle_aabb_sweep_inter,
            circle_poly_sweep_inter,
        ],
        [
            aabb_circle_sweep_inter,
            aabb_aabb_sweep_inter,
            aabb_poly_sweep_inter,
        ],
        [
            poly_circle_sweep_inter,
            poly_aabb_sweep_inter,
            poly_poly_sweep_inter,
        ],
    ];
