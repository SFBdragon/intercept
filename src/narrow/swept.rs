use core::f64;
use crate::narrow::{Aabb, Circle, Intersect, Poly, Shape, ShapeUnion};
...
#[inline]
fn circle_aabb_passby(unit_vel_ca: Vector2<f64>, aabb: &Aabb, o: Vector2<f64>, rad: f64) -> bool {
    // ~3.6ns
...
fn circle_circle_sweep(b1: &Body, c1: &Circle, b2: &Body, c2: &Circle, t: f64) -> Option<(f64, Vector2<f64>)> { // ~12ns(f) - ~19ns(s)
    ...
    let discr = srad * srad + dot * dot - c2c1.magnitude2() as f64;
    if discr > 0.0 {
        let dist = -(dot + discr.sqrt()) * rv_inv_mag;
        if dist >= 0.0 && dist <= 1.0 {
            return Some((dist, (c2c1 + rv.mul_element_wise(dist)).div_element_wise(srad)));
        }
    }
    None
}
        ...
        if differentiator >= -circle.rad && differentiator < aabb_height + circle.rad {
            ...
            return if differentiator < 0.0 {
                ...
                Circle { rad: circle.rad, pos: aabb.min }.line_query(seg_origin, seg_end).map(|travel| 
                    (travel, aabb.min - (seg_origin - rv.mul_element_wise(travel)).div_element_wise(circle.rad)))
                ...
                Circle { rad: circle.rad, pos: aabb.minx_maxy() }.line_query(seg_origin, seg_end).map(|travel| 
                    (travel, aabb.minx_maxy() - (seg_origin - rv.mul_element_wise(travel)).div_element_wise(circle.rad)))
            } else {
        ...
        if differentiator >= -circle.rad && differentiator < aabb_height + circle.rad {
            return if differentiator < 0.0 {
                Circle { rad: circle.rad, pos: aabb.max }.line_query(seg_origin, seg_end).map(|travel| 
                    (travel, aabb.max - (seg_origin - rv.mul_element_wise(travel)).div_element_wise(circle.rad)))
            } else if differentiator > aabb_height {
                Circle { rad: circle.rad, pos: aabb.maxx_miny() }.line_query(seg_origin, seg_end).map(|travel| 
                    (travel, aabb.maxx_miny() - (seg_origin - rv.mul_element_wise(travel)).div_element_wise(circle.rad)))
            } else {
                Some((dx / -rv.x, cgmath::vec2(1.0, 0.0)))
            };
        }
    ...
    if seg_origin.y < aabb.min.y - circle.rad && seg_end.y > aabb.min.y - circle.rad {
        ...
        if differentiator >= -circle.rad && differentiator < aabb_width + circle.rad {
            return if differentiator < 0.0 {
                ...
                Some((dy / -rv.y, cgmath::vec2(0.0, -1.0)))
            };
        }
...
fn aabb_poly_sweep(b1: &Body, aabb: &Aabb, b2: &Body, poly: &Poly, t: f64) -> Option<(f64, Vector2<f64>)> { // 46ns (n=6)
    ...
    if (b1.bullet || b2.bullet) && !aabb_aabb_passby(rv1, &aabb, &poly.aabb, cgmath::vec2(0.0, 0.0)) {
        return None;
    }
...
fn circle_poly_sweep(b1: &Body, circle: &Circle, b2: &Body, poly: &Poly, t: f64) -> Option<(f64, Vector2<f64>)> { // 18(s)-36(s)ns (n=6)
        ...
        if ndotvo >= 0.0 {
            if n.dot(end - v) >= 0.0 {
                ...
                if t < 0.0 {
                    ...
                    let rad_discr = udotvo * udotvo + cr2 - vo_mag2 as f64;
                    if rad_discr > 0.0 {
                        // tangential and passing lines are not collisions
                        let dist = -udotvo - rad_discr.sqrt();
                        return if dist < rv_mag {
                            // impale/fallshort : success/guaranteed failure
                            let norm = (unit_rv.mul_element_wise(dist) - vo).div_element_wise(circle.rad);
                            Some((dist, norm))
                        } else {
                            None
                        };
                    ...
                    let rad_discr = udotv1o * udotv1o + cr2 - v1o.magnitude2() as f64;
                    if rad_discr > 0.0 {
                        let dist = -udotv1o - rad_discr.sqrt();
                        return if dist < rv_mag {
                            let norm = (unit_rv.mul_element_wise(dist) - v1o).div_element_wise(circle.rad);
                            Some((dist, norm))
                        } else {
                            None
                        };
    ...
    let udotvo = (-unit_rv).dot(vo) as f64;
    let rad_discr = udotvo * udotvo + cr2 - vo.magnitude2() as f64;
    if rad_discr > 0.0 {
        // tangential and passing lines are not collisions
        let dist = -udotvo - rad_discr.sqrt();
        if dist < rv_mag {
...
fn shape_sweep(b1: &Body, s1: usize, b2: &Body, s2: usize, t: f64) -> Option<(f64, Vector2<f64>)> {
    ...
    fn circle_circle_sweep_inter(b1: &Body, su1: &ShapeUnion, b2: &Body, su2: &ShapeUnion, t: f64) -> Option<(f64, Vector2<f64>)> {
        unsafe { circle_circle_sweep(b1, &su1.circle, b2, &su2.circle, t) }
    }
    fn aabb_aabb_sweep_inter(b1: &Body, su1: &ShapeUnion, b2: &Body, su2: &ShapeUnion, t: f64) -> Option<(f64, Vector2<f64>)> {
        unsafe { aabb_aabb_sweep(b1, &su1.aabb, b2, &su2.aabb, t) }
    }
    fn poly_poly_sweep_inter(b1: &Body, su1: &ShapeUnion, b2: &Body, su2: &ShapeUnion, t: f64) -> Option<(f64, Vector2<f64>)> {
        unsafe { poly_poly_sweep(b1, &su1.poly, b2, &su2.poly, t) }
    }
    fn circle_aabb_sweep_inter(b1: &Body, su1: &ShapeUnion, b2: &Body, su2: &ShapeUnion, t: f64) -> Option<(f64, Vector2<f64>)> {
        unsafe { circle_aabb_sweep(b1, &su1.circle, b2, &su2.aabb, t) }
    }
    fn circle_poly_sweep_inter(b1: &Body, su1: &ShapeUnion, b2: &Body, su2: &ShapeUnion, t: f64) -> Option<(f64, Vector2<f64>)> {
        unsafe { circle_poly_sweep(b1, &su1.circle, b2, &su2.poly, t) }
    }
    fn aabb_circle_sweep_inter(b1: &Body, su1: &ShapeUnion, b2: &Body, su2: &ShapeUnion, t: f64) -> Option<(f64, Vector2<f64>)> {
        unsafe { circle_aabb_sweep(b2, &su2.circle, b1, &su1.aabb, t).map(|r| (r.0, -r.1)) }
    }
    fn aabb_poly_sweep_inter(b1: &Body, su1: &ShapeUnion, b2: &Body, su2: &ShapeUnion, t: f64) -> Option<(f64, Vector2<f64>)> {
        unsafe { aabb_poly_sweep(b1, &su1.aabb, b2, &su2.poly, t) }
    }
    fn poly_circle_sweep_inter(b1: &Body, su1: &ShapeUnion, b2: &Body, su2: &ShapeUnion, t: f64) -> Option<(f64, Vector2<f64>)> {
        unsafe { circle_poly_sweep(b2, &su2.circle, b1, &su1.poly, t).map(|r| (r.0, -r.1)) }
    }
    fn poly_aabb_sweep_inter(b1: &Body, su1: &ShapeUnion, b2: &Body, su2: &ShapeUnion, t: f64) -> Option<(f64, Vector2<f64>)> {
...
// todo: change all of the manual Body {} init to body::new(), change assert_eq to assert, update API breakages
/*#[cfg(test)]
mod tests {
    use cgmath::vec2;
    use super::*;
